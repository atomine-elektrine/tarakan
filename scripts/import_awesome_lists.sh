...
#
# Lives in the server repo (ops helper), not the Go client.
# Prerequisites: tarakan client on PATH, tarakan login (or TARAKAN_URL + TARAKAN_API_TOKEN).
...
if [[ -z "${TARAKAN_BIN}" ]]; then
  if command -v tarakan >/dev/null 2>&1; then
    TARAKAN_BIN="$(command -v tarakan)"
  elif [[ -x "${HOME}/tarakan-client/bin/tarakan" ]]; then
    TARAKAN_BIN="${HOME}/tarakan-client/bin/tarakan"
  elif [[ -x "${HOME}/.local/bin/tarakan" ]]; then
    TARAKAN_BIN="${HOME}/.local/bin/tarakan"
  else
    echo "error: tarakan client binary not found on PATH (build/install tarakan-client first)" >&2
    echo "  or set TARAKAN_BIN=/path/to/tarakan" >&2
    exit 1
  fi
fi
