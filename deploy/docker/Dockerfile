...
#       zombies.
RUN apt-get update \
  && apt-get install -y --no-install-recommends libstdc++6 openssl libncurses6 locales ca-certificates git tini \
  && rm -rf /var/lib/apt/lists/*
...
# Persistent storage for hosted repositories and the mirror hot-tier. These
# are mounted as volumes in production; creating them owned by nobody means a
# fresh named volume inherits writable ownership.
RUN mkdir -p /app/storage/mirrors /app/storage/hosted \
