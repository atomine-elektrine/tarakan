defmodule TarakanWeb.API.ScanControllerTest do
  ...
  test "rejects requests without a token", %{conn: conn} do
    conn = post(conn, ~p"/api/github.com/openai/codex/scans", scan_body())
  ...
  test "rejects requests with an invalid or revoked token", %{conn: conn, account: account} do
    conn1 = conn |> authed("garbage") |> post(~p"/api/github.com/openai/codex/scans", scan_body())
    ...
    conn2 = conn |> authed(old_token) |> post(~p"/api/github.com/openai/codex/scans", scan_body())
    assert json_response(conn2, 401)
  end

  test "404s for a repository not in the registry", %{conn: conn, token: token} do
    conn = conn |> authed(token) |> post(~p"/api/github.com/unknown/repo/scans", scan_body())
  ...
  test "does not disclose a quarantined repository to an unrelated credential", %{
    ...
    conn =
      ...
      |> post(~p"/api/github.com/openai/codex/scans", scan_body())

    assert json_response(conn, 404)["error"] =~ "not registered"
  end

  test "records a clean scan", %{conn: conn, token: token} do
    conn = conn |> authed(token) |> post(~p"/api/github.com/openai/codex/scans", scan_body())
  ...
  test "records a scan with findings", %{conn: conn, token: token} do
    ...
    conn = conn |> authed(token) |> post(~p"/api/github.com/openai/codex/scans", body)

    assert json_response(conn, 201)["findings_count"] == 2
  ...
  test "requires the scan document", %{conn: conn, token: token} do
    ...
    conn1 = conn |> authed(token) |> post(~p"/api/github.com/openai/codex/scans", body)
    ...
    conn2 =
      conn
      |> authed(token)
      |> post(~p"/api/github.com/openai/codex/scans", scan_body(%{"document" => "[]"}))
  ...
  test "blocks retrying the same run id", %{conn: conn, token: token} do
    ...
    conn2 = conn |> authed(token) |> post(~p"/api/github.com/openai/codex/scans", body)
  ...
  describe "GET /scans and verdict" do
    ...
    test "reviewer-tier reviews:read token sees restricted findings", %{
      ...
      body =
        conn
        |> authed(token)
        |> get(~p"/api/github.com/openai/codex/scans")
        |> json_response(200)

      entry = Enum.find(body["scans"], &(&1["id"] == scan.id))
    ...
    test "a plain findings:submit token cannot see restricted findings", %{
      ...
      refute Enum.any?(body["scans"], &(&1["id"] == scan.id and &1["findings"] != []))
    end
    ...
    test "records a verdict with a proof-of-concept", %{
      ...
      body =
        conn
        |> authed(token)
        |> post(~p"/api/github.com/openai/codex/scans/#{scan.id}/verdict", %{
          "verdict" => "confirmed",
          "provenance" => "agent",
